;; word wrap
; lets experiment without word wrap for a while, lines shouldn't be allowed to get that long
...
(evil-collection-init '(helm minibuffer calc))

;; load any local themes
(with-eval-after-load 'powerline
  ;; load any local themes
  (add-to-list 'custom-theme-load-path "~/.emacs/")
  (load-theme 'nosferatu t))

;; I've got a large screen, lets use it
...
;; yank file link to github
(spacemacs/set-leader-keys "fyg" 'magit-remote-link-copy-at-point)
...
(spacemacs/set-leader-keys "bb" 'spacemacs/persp-buffers)
(spacemacs/set-leader-keys "bB" 'helm-buffers-list)
(spacemacs/declare-prefix "P" "Perspective")
(spacemacs/set-leader-keys "Pn" 'persp-add-new)
(spacemacs/set-leader-keys "PP" 'spacemacs/helm-perspectives)
(spacemacs/set-leader-keys "Ps" 'persp-save-state-to-file)
(spacemacs/set-leader-keys "Pl" 'persp-load-state-from-file)
...
(advice-add 'lsp-sorbet--build-command :override
            (lambda () (pcase (project-name (project-current))
                        ...
                        (t lsp-sorbet--base-command
                         ;(cl-concatenate 'list lsp-sorbet--base-command
...
(font-lock-add-keywords 'ruby-ts-mode
                        '(("#[:|].*" 0 'font-lock-comment-delimiter-face t)))

(with-eval-after-load 'lsp-mode
  (push '("\\.rbi$" "ruby") lsp-language-id-configuration))
...
(setq-default org-capture-templates
      '(("w" "WI" entry
        ...
        ("j" "Journal Entry" entry
         (file "~/workorg/journal.org")
         (file "/Users/warnock/workorg/org_capture/journal_entry.org"))))
(setq-default org-capture-bookmark nil)

;; return a top level week heading if it's a Monday
(defun journal--maybe-week-heading ()
  (if (string-equal "Monday" (format-time-string "%A"))
      (format-time-string "* Week of %Y-%m-%d\n")))

;; pull in any todo entries marked with a [>] from the previous day
(defun journal-pull-bumped-todos ()
  (interactive)
  (with-current-buffer "journal.org"
    (goto-char (point-max))
    (let* ((previous-day (org-get-entry))
          (bumped-todos (cl-remove-if-not (lambda (entry)
                                            (string-match-p "- \\[>]" entry))
                                          (split-string previous-day "\n"))))
      (string-join (mapcar (lambda (todo)
                            (string-replace
                              "- [>]" "- [ ]"
                              (string-clean-whitespace todo)))
                          bumped-todos)
                  "\n"))))
